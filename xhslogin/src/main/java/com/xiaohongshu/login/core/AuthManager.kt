...
import com.xiaohongshu.login.model.*
...
class AuthManager(
    ...
    private fun exchangeCodeForToken(code: String) {
        ...
        val request = TokenRequest(
            ...
            redirectUri = null, // redirectUri 在新流程中不需要
            codeVerifier = helper.codeVerifier
        )
        
        authService.getAccessToken(request, object : ApiCallback<TokenResponse> {
    ...
    private fun getUserInfo(accessToken: String) {
        authService.getUserInfo(accessToken, object : ApiCallback<UserInfoResponse> {
    ...
    fun refreshToken(callback: XHSUserCallback) {
        ...
        authService.refreshToken(refreshToken!!, appId, appSecret, object : ApiCallback<TokenResponse> {
            override fun onSuccess(tokenResponse: TokenResponse) {
                saveTokenInfo(tokenResponse)
